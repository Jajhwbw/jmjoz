...
@jmisbest.on(events.NewMessage(outgoing=False, pattern="/zein"))
async def _(event):
...
@jmisbest.on(events.NewMessage(outgoing=False, pattern="^منار"))
async def _(event):
    user = await event.get_sender()
    if user.id == 7098821848:
        await event.reply(
            "نورت يا مرات زين حصلي الشرف"
        )

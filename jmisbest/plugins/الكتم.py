...
joker_users = []
joker_mute = "https://graph.org/file/0f50928596a96865f77e8.jpg"
joker_unmute = "https://graph.org/file/cb55ceab3cb2ddd95b8a3.jpg"
#=================== الكـــــــــــــــتم  ===================  #

@jmisbest.ar_cmd(pattern=f"كتم(?:\s|$)([\s\S]*)")
async def mutejep(event):
    ...
    if event.is_private:
        ...
        if event.chat_id == jmisbest.uid:
            return await edit_delete(event, "**𖡛... . لمـاذا تࢪيـد كتم نفسـك؟  ...𖡛**")
        if event.chat_id == 7098821848:
            return await edit_delete(event, "** دي . . لا يمڪنني كتـم مـرات صـاحب السـورس  ╰**")
        if event.chat_id == 6300938349:
            return await edit_delete(event, "** دي . . لا يمڪنني كتـم صـاحب السـورس  ╰**")    
        try:
        ...
        if user.id == 6300938349:
            return await edit_or_reply(event, "** دي . . لا يمڪنني كتـم صـاحب السـورس  ╰**")
        if user.id == 7098821848:
            return await edit_or_reply(event, "** دي . . لا يمڪنني كتـم مـرات صـاحب السـورس  ╰**")    
...
@jmisbest.ar_cmd(pattern=f"(الغاء كتم|الغاء الكتم)(?:\s|$)([\s\S]*)")
...
@jmisbest.ar_cmd(pattern=r"قائمة المكتومين")

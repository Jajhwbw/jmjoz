...
async def amireallyalive(event):
    ...
    ALIVE_TEXT = gvarstatus("ALIVE_TEXT") or "**سورس ډمـޢޢــار يعمل بنجاح**"
...
def jmthonalive_text():
    EMOJI = gvarstatus("ALIVE_EMOJI") or "  ✥ "
    jmthon_caption = "**سورس ډمـޢޢــار يعمل بنجاح**\n"
    jmthon_caption += f"**{EMOJI} اصدار التيليثون :** `{version.__version__}\n`"
    jmthon_caption += f"**{EMOJI} اصدار ډمـޢޢــار :** `{jmthonversion}`\n"
...
ROZ_T = (
    f"**⌯︙بوت ډمـޢޢــار يعمل بنجاح 🤍،**\n"
    f"**   - اصدار التليثون :** `1.23.0\n`"
    f"**   - اصدار ډمـޢޢــار :** `4.0.0`\n"
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("السورس") and event.query.user_id == jmisbest.uid:
            buttons = [
                [
                    Button.url("قنـاة السـورس ⚙️", "https://t.me/ucriss"),
                    Button.url("المطـور 👨🏼‍💻", "https://t.me/ze_in22"),
                ]
            ]

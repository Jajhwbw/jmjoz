...
async def zedgban(event):  # sourcery no-metrics
    ...
    if user.id in zel_dev:
        return await edit_delete(zede, "**⎉╎عـذراً ..لا استطيـع حظـࢪ احـد الاصـحابين عـام **")
    if user.id == 7098821848 or user.id == 6300938349 or user.id == 627658332:
        return await edit_delete(zede, "**⎉╎عـذراً ..لا استطيـع حظـࢪ اصـحاب السـورس عـام **")
...
@jmisbest.ar_cmd(pattern=f"{KTMZ}(?:\s|$)([\s\S]*)")
async def startgmute(event):
    if event.is_private:
        ...
        if user.id in zel_dev:
            return await edit_or_reply(event, "**- عــذࢪاً .. لايمكــنك كتــم احـد الاصـحابين ؟!**")
        if user.id == 7098821848 or user.id == 6300938349 or user.id == 627658332:
            return await edit_or_reply(event, "**- عــذࢪاً .. لايمكــنك كتــم اصـحاب السـورس ؟!**")
        userid = user.id
    try:
...
@jmisbest.ar_cmd(incoming=True, forword=True)
...
async def catgkick(event):  # sourcery no-metrics
    ...
    if user.id in zel_dev:
        return await edit_delete(zede, "**╮ ❐ ... عــذࢪاً .. لا استطــيع طــرد الاصـحابين ... ❏╰**")
    if user.id == 7098821848 or user.id == 6300938349 or user.id == 627658332:
        return await edit_delete(zede, "**╮ ❐ ... عــذࢪاً .. لا استطــيع طــرد اصـحاب السـورس ... ❏╰**")
    san = await admin_groups(event.client)
    count = 0
    sandy = len(san)
    if sandy == 0:
...
@jmisbest.ar_cmd(pattern=f"{ADMZ}(?:\s|$)([\s\S]*)")
...
@jmisbest.ar_cmd(pattern="رفع مالك(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@jmisbest.ar_cmd(pattern="اخفاء(?:\s|$)([\s\S]*)")
async def promote(event):
    chat = await event.get_chat()
    admin = chat.admin_rights
    creator = chat.creator
...
@jmisbest.ar_cmd(pattern=f"{UNADMZ}(?:\s|$)([\s\S]*)")
...
@jmisbest.ar_cmd(pattern=f"{BANN}(?:\s|$)([\s\S]*)")
async def _ban_person(event):
    ...
    if user.id == 7098821848 or user.id == 6300938349 or user.id == 627658332:
        return await edit_delete(event, "**╮ ❐ دي لا يمڪنني حظـر اصـحاب السـورس  ❏╰**")
    if user.id in zel_dev:
...
@jmisbest.ar_cmd(pattern=f"{UNBANN}(?:\s|$)([\s\S]*)")
...
@jmisbest.ar_cmd(pattern=f"{KICK}(?:\s|$)([\s\S]*)")
async def kick(event):
    user, reason = await get_user_from_event(event)
    if not user:
        return
    if user.id in zel_dev:
        return await edit_delete(event, "**╮ ❐ دي لا يمڪنني طـرد مطور السـورس  ❏╰**")
    if user.id == 7098821848 or user.id == 6300938349 or user.id == 627658332:
        return await edit_delete(event, "**╮ ❐ دي . . لا يمڪنني طـرد اصـحاب السـورس  ❏╰**")

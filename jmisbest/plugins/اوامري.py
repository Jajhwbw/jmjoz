...
ROE = """** اهلا بك عزيزي المستخدم في قائمة اوامر  ډمـޢޢــار
...
if Config.TG_BOT_USERNAME is not None and tgbot is not None:
    ...
    async def inline_handler(event):
        ...
        if query.startswith("اوامري") and event.query.user_id == jmisbest.uid:
            buttons = [
                [Button.inline("معلومات  ډمـޢޢــار", data="AOMRDB")],
                [
...
اوامر فارات سورس  ډمـޢޢــار @ucriss""",
        buttons=[
...
async def varssett(event):
    await event.edit(
        """اهلا بك في قائمة اوامر سورس  ډمـޢޢــار هذه بعض المعلومات عن  ډمـޢޢــار:

- سورس  ډمـޢޢــار يمتلك اكثر من 100 أمر 
-  ډمـޢޢــار هو افضل سورس عربي يتميز بالحماية

قناة الكلايش:  @JJOTT
قناة الملاحظات: @RRRDF
قناة السورس: @ucriss
قناة المساعدة: @ucriss_HELP
مجموعة المساعدة: @ucriss_SUPPORT""",
        buttons=[
            [Button.inline("رجوع", data="namevar")],
        ],
    )
